...
//        window.setGravity(Gravity.BOTTOM);//设置在底部
	...
	private void initView() {
		mYear = myView.findViewById(R.id.tv_year);
		mMonthDay = myView.findViewById(R.id.tv_monthday);
		mCalendarView =  myView.findViewById(R.id.calendarView);
		mCancleBtn = myView.findViewById(R.id.tv_cancel);
		mOkBtn =  myView.findViewById(R.id.tv_ok);
	}
	...
	private void initData() {
		...
		mCalendarView.state().edit()
				.setMinimumDate(selectedDate)//设置今天以前的日期不可选
				.commit();
		mCalendarView.addDecorators(new MySelectorBgDecorator(mContext), selectedDayDecorator, new TodayDecorator(mContext));
